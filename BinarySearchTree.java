...
public class BinarySearchTree<T extends Comparable<? super T>> {
    ...
    public void insert(T x) {
        if (this.find(x) == x) {
            throw new DuplicateItemException("gpa cannot lower 2.0");
        }
        root = insert(x, root);
    }
...

    {
        ...
        if (this.find(x) == null) {
            throw new ItemNotFoundException("gpa cannot lower 2.0");
        }
        root = remove(x, root);
    }
    ...
    public T findMin() {
        ...
        return this.elementAt(findMin(root));
    }
...

    {
        ...
        return this.elementAt(findMax(root));
    }
    ...
    private BinaryNode<T> remove(T x, BinaryNode<T> node) {
        ...
        else 
        {
            ...
            if (node.getLeft() != null && node.getRight() != null) {
                T min = this.elementAt(this.findMin(node.getRight()));
                node.setElement(min);
